/* Forecourt — offline shell.
   Keeps the app openable on a forecourt with one bar of signal.
...
self.addEventListener("fetch", e => {
  ...
  // The typeface is part of the shell — cache it so the app looks right offline.
  if (FONT_HOSTS.has(url.origin)) { e.respondWith(swr(req)); return; }
  // Anything else off-origin (i.e. Supabase) must always go to the network.
